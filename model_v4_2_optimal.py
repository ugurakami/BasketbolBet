"""
Basketball Betting Analytics System - MODEL V4.2 (Optimal & Final)
Author: AI Assistant
Description: Optimal ML system combining Regressor-based Limit Line prediction and Calibrated Classifiers for Confidence-Based Staking. This version is prepared for performance comparison (A/B testing) in a GitHub project.
...
from sklearn.calibration import CalibratedClassifierCV # Integrated calibration
import warnings
...
def validate_model_quality(side_accuracy, over_accuracy, score_r2):
    """Checks model quality against thresholds and prints a warning if performance is low."""
...
def send_telegram_message(df_bets, analysis_date):
    """Sends the analysis results via Telegram."""
 ...
            limit_line_int = int(round(limit_line / 2.5) * 2.5) # Rounded Limit Line
...
def calibrate_limit_line(predicted_scores):
    """Rounds the limit line to the nearest 2.5 multiple and applies clipping."""
...
def calculate_team_offensive_rating(team, date, historical_data, n_matches):
    """Calculates offensive and defensive rating based on recent matches."""
...
def calculate_team_form(team, date, historical_data, n_matches):
    """Calculates win rate and First Half Scoring Ratio."""
    home_matches = historical_data[
        (historical_data['Home_Team'] == team) &
        (historical_data['Date'] < date)
    ].tail(n_matches)
 ...
    # First Half Ratio (Yeni: İlk Yarı Oranı)
 ...
    fh_score_ratio = total_fh_score / total_score if total_score > 0 else 0.5
...
def create_features(df, historical_data, n_matches):
    """Creates comprehensive features for model training, handling time-dependency safely."""
    df_features = df.copy()
 ...
    # 2. Advanced Team Ratings & Form/Ratio - Row-wise calculation
    ratings_data = []
 ...
    )
    
    # Drop unused columns and concatenate new features
    columns_to_drop = [col for col in ['Home_Stats', 'Away_Stats'] if col in df_features.columns]
    df_features = pd.concat([df_features.drop(columns=columns_to_drop), ratings_df], axis=1)

    # 3. Expected Total Score and Strength Differences
...
def load_and_clean_data(file_path):
    """Loads, cleans, and separates historical/future data."""
    try:
 ...
    # Parse First Half scores (Yeni: İlk Yarı skorlarını temizle ve dönüştür)
 ...
    # Completion mask: Final score, Half time score, and Date must be present.
    mask_completed = (
 ...
    df_future = df[~mask_completed].copy()
...
def prepare_model_data(df, n_matches):
    """Prepares features, targets, and splits data for ML."""
 ...
    # Numeric features for Regressor
...
def train_and_evaluate_models(X_train_full, X_train_num, y_side, y_over, y_total_score):
    """Trains three models and evaluates them using TimeSeriesSplit."""
...
def find_valuable_bets(predictions_df, min_probability, margin_threshold):
    """Identifies valuable bets based on model confidence and margin to the limit line."""
    valuable_bets = []
...
def main():
 ...
    # 1. Limit Line Prediction (Regressor Model)
 ...
    # 2. Classification and Calibration
 ...
        # Find valuable bets (Risk and Margin Check)
